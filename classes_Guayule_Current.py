...
class SoilClass:
    ...
    **Attributes**:\n
    ...
    def __init__(self,soilType,dz=[0.1]*12,
        ...
        if soilType == 'custom':
        ...
        elif soilType == 'SandyLoam2018': # data from 2018
            self.CN = 61 
            self.CalcCN = 0
            self.REW = 7 
            dz = [0.1]*5 + [0.15]*2 + [0.3]*5  # thickness of each soil compartment e.g. 12 compartments of thickness 0.1m -> want 0.2 * 7 
            # The total thickness our layers have is 0.2m*7 = 1.4m. Inclusing the evaporation layer you have 1.6m
            self.create_df(dz)
            ...
            #specifying soil hydraulic properties
            # Had to make first 4 layers repeated at 0.1m, last 4 layers are only taken once at 0.2m 
            self.add_layer(0.5, 0.142, 0.237, 0.384, 132, 100) # Evaporation layer
            self.add_layer(1.8, 0.132, 0.231, 0.387, 383, 100)
...
class CropClass:
    ...
    **Attributes**:\n
    ...
    def __init__(self,c_name,PlantingDate,HarvestDate=None,**kwargs):
        ...
        self.CCmin = 0.1 # Minimum canopy size below which yield formation cannot occur
...
# 2018 only, pre-irrigation and full treatment
            self.Emergence = 0 # Growing degree/Calendar days from sowing to emergence/transplant recovery -> 10 to 15 days
            self.MaxRooting = 2400 # Growing degree/Calendar days from sowing to maximum rooting -> end of development stage
            self.Senescence = 6000 # Growing degree/Calendar days from sowing to senescence
            self.Maturity = 6000 # Growing degree/Calendar days from sowing to maturity

# Check if percentage or decimal -> I think this is wrong. Should be days not percentage or decimal. Chose a random number tbh 
            self.HIstart = 2000 # Growing degree/Calendar days from sowing to start of yield formation
            
            self.Flowering = -999 # Duration of flowering in growing degree/calendar days (-999 for non-fruit/grain crops)
            self.YldForm = 750 # Duration of yield formation in growing degree/calendar day
        
            self.GDDmethod = 3 # Growing degree day calculation method -> Ask Waller which one he wants 
       
            self.Tbase = 0 # Base temperature (degC) below which growth does not progress
            self.Tupp = 37.78 # Upper temperature (degC) above which crop development no longer increases
...
# Used soil temperatures from https://www.cropj.com/gresta_12_6_2018_954_960.pdf for germination 
            self.Tmax_up = 32.22 # Maximum air temperature (degC) above which pollination begins to fail -> (37) Estimation based on PDF pg 955 
            self.Tmax_lo = 43.33 # Maximum air temperature (degC) at which pollination completely fails -> Estimation based on above value
            ...
            self.Tmin_lo = 10 # Minimum air temperature (degC) at which pollination completely fails
            self.BioTempStress = 1 # Biomass production affected by temperature stress (0: No; 1: Yes) 
            self.TrColdStress = 1 # Transpiration affected by cold temperature stress (0 = No, 1 = Yes)
            
# Check later 
            self.GDD_up = 1 # Minimum growing degree days (degC/day) required for full crop transpiration potential
            self.GDD_lo = -2.5 # Growing degree days (degC/day) at which no crop transpiration occurs
        
            self.Zmin = 0.3 # Minimum effective rooting depth (m)
            self.Zmax = 2.1 # Maximum rooting depth (m)
...
# They are sown 2-3 cm deep into the soil https://pfaf.org/user/Plant.aspx?LatinName=Cyamopsis and the seeds themselves are around 5 mm (.5 cm) long, then you have about 4.3 cm^2 surface area (using 2.5 cm height and 0.25 radius of the seed) -> SA of cylinder
            ...
            self.PlantPop = 27_000 # Number of plants per hectare KEEP IN MIND THE _ (24_000)
            self.CCx = 0.9 # Maximum canopy cover (fraction of soil cover)
            
# Estimate / look up -> Slow decline, use a small fraction 
            self.CDC = 0.001 # Canopy decline coefficient (fraction per GDD/calendar day)
            self.CGC = 0.01 # Canopy growth coefficient (fraction per GDD)
            
            self.Kcb = 1.3 # Crop coefficient when canopy growth is complete but prior to senescence
# Only 20 days, estimate 10 or 15%
            self.fage = 0 #  Decline of crop coefficient due to ageing (%/day)
...
# Estimate 100% 
            ...
            self.fsink = 0.2 # Crop performance under elevated atmospheric CO2 concentration (%/100)
            self.HI0 = 0.07 # Reference harvest index
